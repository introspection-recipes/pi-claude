/**
 * Web access for the Claude Code recipe.
 *
 * Pi ships no built-in web tools and has no native web search, so this
 * extension provides both Claude Code web capabilities as real client-side
 * tools, reusing Claude Code's own tool prompts:
 *
    ...
    async execute(_toolCallId, params, signal) {
      ...
      try {
        const res = await fetch(url, {
          signal,
          redirect: "follow",
          headers: { "User-Agent": "claude-code-pi-recipe/0.1 (+webfetch)" },
        });
