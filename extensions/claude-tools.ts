/**
 * Claude Code agent-facing tools for the Pi runtime.
 *
 * Pi ships the core file/search/shell tools (read, edit, write, grep, find,
 * bash). Claude Code's harness adds a `todo_write` task-list tool that the
 * system prompt instructs the agent to use for planning multi-step work. This
 * extension registers it so the recipe behaves like Claude Code.
 */
