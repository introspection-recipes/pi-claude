...
introspection init claude-code-agent template-claude
...
> Inspect this repository, identify the main validation command, and propose a short plan. Do not change any files.
...
3. Choose the repository and the runtime in `.introspection/claude-code-agent.yaml`, confirm that its recipe path is `.`, and create the first version from `main`.
4. If you need `WebSearch`, configure the `PARALLEL_API_KEY` variable bindings described below. Bindings are environment-specific.
...
Skip this section if you do not need `WebSearch`. The extension reads `PARALLEL_API_KEY` from the runtime sandbox, so the Development lane needs a variable binding before its first search:

1. In the Introspection app, open **Runtimes**, select `claude-code-agent`, and open **Bindings**.
2. In the **Variables** section, create a variable named `PARALLEL_API_KEY` and paste your Parallel API key as its value.
3. Scope it to the `claude-code-agent` runtime group and the **Development** environment, then save it. Confirm that the resulting row is labeled as a variable for Development.
...
introspection bindings variables create \
  ...
  --runtime-group claude-code-agent \
...
```bash
introspection dev --runtime claude-code-agent
```

Leave the command running, open the development chat URL it prints, and repeat the inspection-and-plan prompt. Saved recipe changes are picked up without a commit or push. The development task resolves the Development-scoped `PARALLEL_API_KEY` variable you created above; `introspection dev` does not read or upload the key exported for a local Pi session. Stopping the command removes the local overlay and does not deploy a version.
...
In the runtime's **Versions** view, find the candidate by branch and commit, pin it to **Staging**, and select **Preview**. Repeat the inspection-and-plan prompt. If your change affects search, also ask a current question and confirm the Parallel binding works. Verify that the conversation used the intended candidate commit and completed without an unhandled tool or binding error.
...
Run one small production check through the stable `claude-code-agent` runtime and confirm that it resolves to the merged recipe commit.
...
- Runtime metadata in `.introspection/claude-code-agent.yaml`
